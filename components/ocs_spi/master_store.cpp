...
MasterStore::MasterStore(MasterStore::Params params) {
    ...
    config.mosi_io_num = params.mosi;
    config.miso_io_num = params.miso;
    config.sclk_io_num = params.sclk;
    config.quadwp_io_num = -1;
    config.quadhd_io_num = -1;
    config.max_transfer_sz = params.max_transfer_size;

    ESP_ERROR_CHECK(spi_bus_initialize(SPI3_HOST, &config, SPI_DMA_DISABLED));
}

IStore::ITransceiverPtr
MasterStore::add(const char* id, gpio_num_t cs, IStore::TransferSpeed speed) {
    configASSERT(id);
    ...
    auto err = spi_bus_add_device(SPI3_HOST, &config, &device);
    if (err != ESP_OK) {
